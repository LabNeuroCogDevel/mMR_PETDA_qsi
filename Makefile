...
txt/best_fmap_for_dwi.txt: dwi-mmr-fm.R ${DWI} ${FMAP}
	./01_dwi-mmr-fm.R
txt/best_T1w_for_dwi.txt: dwi-mmr-t1.R ${DWI} ${ANAT}
	./01_dwi-mmr-t1.R

# rename and modify json
.make/phasediff.ls: add_intended.bash txt/best_fmap_for_dwi.txt
...
.make/intended_only.ls: txt/best_T1w_for_dwi.txt .make/phasediff.ls
	...
	mkls $@ 'BIDS_FMP/sub-*/ses-*/{anat,fmap}/*.{nii.gz,json}' 
