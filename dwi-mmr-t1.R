...
cmd <- "
find  /Volumes/Hera/Raw/BIDS/mMRDA-dev/ -iname '*G2*T1w*.json' -or -iname '*dwi.json' |
while read f; do 
jq .AcquisitionTime < $f |
sed \"s:^:$(basename $f) :\"; done |
   perl -lne '
      print \"$1 $3$2 $4\" if m/(sub-.*)_(acq-dwi|dwi|acq-[12]ADNI).*?(run-\\d+|\\.).*?json \"(.*)\"/
   '"
...
d <-
  ...
  group_by(id) %>% 
  mutate(acqdiff=abs(dwi-acqtime), n_ses_T1w=n()) %>%
  filter(max(acqtime)==acqtime)
...
write.table(d, file="txt/best_T1w_for_dwi.txt", row.names=F, quote=F, sep="\t")
